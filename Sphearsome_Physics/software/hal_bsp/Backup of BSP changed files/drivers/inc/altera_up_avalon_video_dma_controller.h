...
/**
 * @brief Read a symbol at the location specified by <em>(x, y)</em> on the VGA monitor
 *
 * @param video_dma		-- the pointer to the Video DMA device structure
 * @param x				-- the \em x coordinate
 * @param y				-- the \em y coordinate
 * @param backbuffer	-- set to 1 to select the back buffer, otherwise set to 0 to select the current screen.
 *
 * @return symbol for success, -1 for error (such as out of bounds)
 **/
unsigned int alt_up_video_dma_read(alt_up_video_dma_dev *video_dma,
		unsigned int x, unsigned int y, int backbuffer);
...
/**
 * @brief This function draws a filled size-pixel-wide circle of a given symbol .
 *
 * @param video_dma		-- the pointer to the Video DMA device structure
 * @param symbol		-- the symbol to be drawn
 * @param x0,y0			-- coordinates of the top left (x0,y0) and bottom right (x1,y1) corner of the box
 * @param circle		-- two dimensional circle array of 0s and 1s
 * @param backbuffer	-- set to 1 to select the back buffer, otherwise set to 0 to select the current screen.
 *
 * @return 0 for success, -1 for error (such as out of bounds)
 **/
